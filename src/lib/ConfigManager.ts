...
class ConfigManager
{
	...
	public render():void
	{
		...
		this._parsedConfig = <IConfig>deepAssign.apply(null, envs);
		...
		var getVar = (varName:string):string =>
		{
			...
			{
				return variable['replace'](this._varRegExp, (result:string, match:string) =>
				{
					// recursion
					return getVar(match);
				});
		...
		for (var x = 0; x < types.length; x++)
		{
			...
			for (var i in vars)
			{
				...
				{
					config = currentVar.replace(this._varRegExp, (result:string, match:string) =>
					{
						return getVar(match);
					});
				}
	...
	{
		return subject['replace'](this._varRegExp, (result:string, match:string) =>
		{
